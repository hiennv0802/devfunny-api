        ...
        request(app)
          ...
          .then((res) => {
            request(app)
              ...
              .then((resSection) => {
                expect(resSection.body.success).to.equal(false);
                expect(resSection.body.message).to.equal('No file was selected');
                done();
              })
              .catch(done);
          })
        ...
        request(app)
          ...
          .then((res) => {
            request(app)
              ...
              .expect(httpStatus.OK)
              .then((resSection) => {
                expect(resSection.body.name).to.equal('funny');
                expect(resSection.body).to.have.property('image');
                done();
              })
              .catch(done);
          })
